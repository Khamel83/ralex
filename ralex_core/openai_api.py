...
def setup_websocket_handlers():
    ...
    async def handle_budget_add_request(data):
        ...
        if result:
            ...
            await websocket_manager.send_to_connection(
                connection_id,
                {"type": "error", "data": {"error": "Failed to add budget"}},
            )
    ...
    async def handle_file_add_request(data):
        ...
        if session and agentos:
            agentos.add_file_to_session(
                session_id, file_path, "# File content", {"type": "code"}
            )
            ...
            await websocket_manager.send_to_connection(
                connection_id,
                {"type": "error", "data": {"error": "Failed to add file to context"}},
            )
    ...
    async def handle_session_info_request(data):
        ...
        if session:
            ...
            await websocket_manager.send_to_connection(
                connection_id, {"type": "session_info", "data": session_summary}
            )
...
def get_or_create_session(user: Optional[str] = None) -> str:
    ...
    session_id, session = session_manager.get_or_create_session(
        session_id=f"user_{user}" if user else None, initial_budget=5.0
    )
    return session_id
        ...
        if smart_router:
            ...
            if session:
                ...
                selected_model = smart_router.select_model(
                    intent, enhanced_prompt, budget_context
                )
...
async def call_ralex_backend(
    messages: List[ChatMessage], model: str, session_id: str
) -> str:
    ...
    try:
        ...
        response = openrouter_client.create_chat_completion(
            model=model, messages=openrouter_messages, stream=False
        )
        ...
        if response and "choices" in response and len(response["choices"]) > 0:
            ...
            if "usage" in response:
                ...
                estimated_cost = (
                    usage.get("total_tokens", 0) * 0.000002
                )  # Rough estimate
                ...
                if budget_status:
                    ...
                    await websocket_manager.send_budget_update(
                        session_id, budget_status
                    )
...
async def stream_ralex_response(
    messages: List[ChatMessage], model: str, session_id: str
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    await websocket_manager.send_typing_indicator(
        session_id, True, selected_model, "Processing request..."
    )
    await websocket_manager.send_model_selection(
        session_id, selected_model, routing_info["reasoning"]
    )
    ...
    if request.stream:
        ...
        response_content = await call_ralex_backend(
            request.messages, selected_model, session_id
        )
        ...
        if session:
            session.add_message(
                "assistant",
                response_content,
                {"model": selected_model, "intent": routing_info["intent"]},
            )
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    connection_id = await websocket_manager.connect(
        websocket, session_id, user_agent, client_host
    )
