...
class SandboxConfig:
    ...
    def __init__(self, config_dir: str):
        ...
        self.allowed_file_operations = set(
            self.code_execution.get("allowed_file_operations", ["read"])
        )
        self.restricted_paths = set(self.code_execution.get("restricted_paths", []))

        logger.info(
            f"Sandbox initialized: enabled={self.enabled}, sandboxed={self.sandboxed}"
        )
...
class ASTSecurityValidator:
    ...
    def _check_import(self, node: ast.Import) -> Optional[str]:
        ...
        for alias in node.names:
            ...
            if (
                ...
                and module_name not in self.config.allowed_imports
            ):
    ...
    def _check_call(self, node: ast.Call) -> Optional[str]:
        ...
        if isinstance(node.func, ast.Name):
            ...
            if (
                func_name == "open"
                and "read" not in self.config.allowed_file_operations
            ):
...
class SecureSandbox:
    ...
    def execute_python_code(
        self, code: str, globals_dict: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _execute_sandboxed(
        ...
        try:
            ...
            error = (
                f"Code execution timed out after {self.config.timeout_seconds} seconds"
            )
        except MemoryError:
            error = (
                f"Code execution exceeded memory limit ({self.config.max_memory_mb} MB)"
            )
        except Exception as e:
