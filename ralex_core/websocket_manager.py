...
class WebSocketManager:
    ...
    async def disconnect(self, connection_id: str, code: int = 1000, reason: str = ""):
        ...
        try:
            ...
            self.logger.info(
                f"WebSocket disconnected: {connection_id} from session {session_id}"
            )
    ...
    async def broadcast(
        self, message: Dict[str, Any], exclude_sessions: Set[str] = None
    ) -> int:
    ...
    async def handle_client_message(self, connection_id: str, raw_message: str):
        ...
        try:
            ...
            self.logger.error(
                f"Error handling client message from {connection_id}: {e}"
            )
            await self.send_to_connection(
                connection_id,
                {
    ...
    async def send_budget_update(
        self, session_id: str, budget_data: Dict[str, Any]
    ) -> int:
        """Send budget update to session"""
        return await self.send_to_session(
            session_id, {"type": MessageType.BUDGET_UPDATE.value, "data": budget_data}
        )

    async def send_typing_indicator(
        self, session_id: str, is_typing: bool, model: str = "", message: str = ""
    ) -> int:
        ...
        return await self.send_to_session(
            session_id,
            {
                ...
                "data": {"is_typing": is_typing, "model": model, "message": message},
            },
        )

    async def send_model_selection(
        self,
        session_id: str,
        model: str,
        reasoning: str = "",
    ...
    async def send_processing_status(
        self, session_id: str, status: str, progress: float = 0, message: str = ""
    ) -> int:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            ).total_seconds(),  # Would track actual uptime
        }
