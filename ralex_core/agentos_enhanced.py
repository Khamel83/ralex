...
class EnhancedWebFileContext(WebFileContext):
    ...
    def add_file_with_analysis(
        self, file_path: str, content: str = None, metadata: Dict = None
    ):
        """Add file with comprehensive analysis"""
        if content:
            analysis = self.analyze_code_content(file_path, content)
            enhanced_metadata = {**(metadata or {}), "analysis": analysis}
        else:
    ...
    def get_related_files(self, file_path: str) -> List[str]:
        ...
        if file_path in self.files:
            file_analysis = (
                self.files[file_path].get("metadata", {}).get("analysis", {})
            )
...
class EnhancedAgentOSIntegration(AgentOSWebIntegration):
    ...
    def extract_file_references_enhanced(
        ...
        module_patterns = [r"(?:import|from)\s+(\w+)", r"(\w+)\.py", r"module\s+(\w+)"]
    ...
    def build_enhanced_context(
        ...
        context = {
            ...
            "complexity_analysis": self.analyze_complexity_enhanced(
        ...
        context["suggested_standards"] = self.suggest_relevant_standards(
            context["file_references"], context["complexity_analysis"]
        )
    ...
    def analyze_complexity_enhanced(
        self, user_message: str, session_id: str
    ) -> Dict[str, Any]:
        ...
        enhancement_factors = {
            "mentions_multiple_files": len(
                self.extract_file_references_enhanced(user_message)["explicit_files"]
            )
            > 1,
            "mentions_testing": any(
                word in user_message.lower()
                for word in ["test", "testing", "spec", "coverage"]
            ),
            "mentions_refactoring": any(
                word in user_message.lower()
                for word in ["refactor", "restructure", "reorganize"]
            ),
            "mentions_architecture": any(
                word in user_message.lower()
                for word in ["architecture", "design", "pattern", "structure"]
            ),
            "mentions_performance": any(
                word in user_message.lower()
                for word in ["performance", "optimize", "speed", "efficiency"]
            ),
            "mentions_database": any(
                word in user_message.lower()
                for word in ["database", "db", "sql", "query", "model"]
            ),
            "mentions_api": any(
                word in user_message.lower()
                for word in ["api", "endpoint", "route", "service", "client"]
            ),
        ...
        return {
            **base_analysis,
            ...
            "reasoning": self.build_complexity_reasoning(
                base_analysis, enhancement_factors, total_score
            ),
        }
    ...
    def suggest_relevant_standards(
        self, file_references: Dict[str, List[str]], complexity_analysis: Dict[str, Any]
    ) -> List[str]:
    ...
    def build_complexity_reasoning(
        self,
        ...
        if base_analysis["scores"]["complex"] > 0:
            reasons.append(
                f"contains {base_analysis['scores']['complex']} complex keywords"
            )
        if base_analysis["scores"]["analysis"] > 0:
            reasons.append(
                f"requires {base_analysis['scores']['analysis']} analysis tasks"
            )
        ...
        return (
            f"Enhanced score: {enhanced_score}/10. " + ", ".join(reasons)
            if reasons
            else "Standard complexity analysis"
        )
    ...
    def get_languages_in_context(
        self, file_context: EnhancedWebFileContext
    ) -> List[str]:
    ...
    def build_context_summary(
        self, context: Dict[str, Any], file_context: EnhancedWebFileContext
    ) -> str:
        ...
        if session_state["files_in_context"] > 0:
            summary_parts.append(
                f"{session_state['files_in_context']} files in context"
            )
            if session_state["languages_present"]:
                summary_parts.append(
                    f"Languages: {', '.join(session_state['languages_present'])}"
                )
        ...
        if file_refs["explicit_files"]:
            summary_parts.append(
                f"Referenced files: {', '.join(file_refs['explicit_files'][:3])}"
            )

        # Complexity
        complexity = context["complexity_analysis"]
        summary_parts.append(
            f"Complexity: {complexity['complexity']} ({complexity['recommended_tier']} model)"
        )

        # Standards
        if context["suggested_standards"]:
            summary_parts.append(
                f"Relevant standards: {', '.join(context['suggested_standards'][:3])}"
            )
                ...
                if file_info and file_info.get("content"):
                    ...
                    if analysis:
                        prompt_sections.append(
                            f"// Analysis: {analysis.get('language', 'unknown')} file with {len(analysis.get('functions', []))} functions, {len(analysis.get('classes', []))} classes"
                        )
                    prompt_sections.append(
    ...
    def build_response_instructions(self, context: Dict[str, Any]) -> str:
        ...
        if file_refs["explicit_files"]:
            instructions.append(
                "6. Shows both original and modified code sections when editing files"
            )
