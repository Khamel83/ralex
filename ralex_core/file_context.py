...
class FileContextManager:
    ...
    def load_file(
        ...
        try:
            ...
            if file_size > self.max_file_size_mb * 1024 * 1024:
                logger.warning(
                    f"File too large ({file_size / 1024 / 1024:.1f}MB): {file_path}"
                )
                return None
            ...
            try:
                ...
                for fallback_encoding in ["latin-1", "cp1252", "iso-8859-1"]:
                    try:
                        ...
                        logger.debug(
                            f"Used fallback encoding {encoding} for {file_path}"
                        )
                        break
                    except UnicodeDecodeError:
                        continue
    ...
    def load_directory(
        ...
        try:
            ...
            logger.info(
                f"Loaded project: {directory_path} ({len(loaded_files)} files, {list(languages)})"
            )
    ...
    def get_file_context_summary(
        self, files: List[FileInfo], max_chars: int = 10000
    ) -> str:
        """
        ...
        for file_info in sorted_files:
            if current_length >= max_chars * 0.9:  # Leave some room
                ...
                summary_parts.append(
                    f"... and {remaining_count} more files (truncated due to length)\n"
                )
                break
            ...
            remaining_chars = (
                max_chars - current_length - len(file_header) - 100
            )  # Buffer
            ...
            file_section = (
                f"{file_header}```{file_info.language or ''}\n{content}\n```\n\n"
            )
    ...
    def extract_functions_and_classes(
        ...
        try:
            ...
            logger.warning(
                f"Failed to extract functions/classes from {file_info.path}: {e}"
            )
    ...
    def _detect_language(self, file_path: str, content: str) -> Optional[str]:
        ...
        if any(
            indicator in content
            for indicator in ["function", "const ", "let ", "var ", "=>"]
        ):
            return "javascript"
        ...
        if any(
            indicator in content
            for indicator in ["public class", "private ", "public static void main"]
        ):
            return "java"
                ...
                for imp in imports:
                    if imp[0]:  # from X import Y
                        metadata["imports"].append(
                            f"from {imp[0]} import {imp[1].strip()}"
                        )
                    else:  # import X
    ...
    def _generate_directory_structure(
        self, root_path: str, files: List[FileInfo]
    ) -> Dict[str, Any]:
    ...
    def _cache_file_info(self, file_path: str, file_info: FileInfo):
        ...
        if len(self.file_cache) > self.cache_max_size:
            ...
            oldest_key = min(
                self.file_cache.keys(), key=lambda k: self.file_cache[k].last_modified
            )
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        total_size = sum(
            len(info.content.encode("utf-8")) for info in self.file_cache.values()
        )
...
if __name__ == "__main__":
    ...
    project_context = file_manager.load_directory(
        current_dir, recursive=False, include_patterns=["*.py"]
    )
    ...
    if project_context:
        ...
        summary = file_manager.get_file_context_summary(
            project_context.files, max_chars=2000
        )
