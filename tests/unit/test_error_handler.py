...
class TestErrorHandler(unittest.TestCase):
    ...
    def test_get_user_friendly_message_file_not_found(self):
        ...
        self.assertEqual(message, "I couldn't find the specified file. Please check the path and try again.")

    def test_get_user_friendly_message_permission_error(self):
        error = PermissionError("access denied")
        message = self.handler.get_user_friendly_message(error)
        self.assertEqual(message, "I don't have permission to access that file or directory. Please check your permissions.")

    def test_get_user_friendly_message_api_error(self):
        error = Exception("API call failed")
        message = self.handler.get_user_friendly_message(error)
        self.assertEqual(message, "There was an issue connecting to the AI service. Please check your internet connection or API key.")

    def test_get_user_friendly_message_generic_error(self):
        error = TypeError("Invalid type")
        message = self.handler.get_user_friendly_message(error)
        self.assertEqual(message, "An unexpected error occurred. I've logged the details for further investigation.")
