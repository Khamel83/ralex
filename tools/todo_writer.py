...
class TodoWriter:
    ...
    def create_task(
        self, task_id: str, name: str, description: str, priority: str = "medium"
    ) -> Dict[str, Any]:
        """Create a new task"""
    ...
    def update_task(self, task_id: str, **updates) -> Dict[str, Any]:
        ...
        if old_status != "completed" and task.status == "completed":
            self.logger.info(
                f"Task {task_id} marked as completed, creating git commit..."
            )
            git_result = self.git_manager.sync(
                GitSyncManager._create_commit_message(task)
            )
    ...
    def list_tasks(self, status_filter: str = None) -> Dict[str, Any]:
        ...
        return {
            "tasks": {
                task_id: asdict(task) for task_id, task in filtered_tasks.items()
            },
...
def quick_complete_task(
    ...
    return writer.complete_task(
        task_id, verification_steps, files_modified, next_task_info
    )
