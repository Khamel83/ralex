...
class CodeLinter:
    ...
    def __init__(self):
        ...
        self.linters = {
            ...
            "json": {"builtin": self._lint_json_builtin},
            "yaml": {
                "primary": {
                    "name": "yamllint",
        ...
        logger.info(
            f"Code linter initialized with {len(self.available_linters)} available linters"
        )
    ...
    def lint_code(
        ...
        if "primary" in linter_config:
            try:
                issues = self._run_external_linter(
                    code, linter_config["primary"], filename
                )
                linter_used = linter_config["primary"]["name"]
            except Exception as e:
                logger.debug(
                    f"Primary linter {linter_config['primary']['name']} failed: {e}"
                )
                ...
                if "fallback" in linter_config:
                    try:
                        issues = self._run_external_linter(
                            code, linter_config["fallback"], filename
                        )
    ...
    def _run_external_linter(
        self, code: str, linter_config: Dict[str, Any], filename: Optional[str]
    ) -> List[LintIssue]:
    ...
    def _lint_java_builtin(
        ...
        for i, line in enumerate(lines, 1):
            ...
            if (
                any(keyword in stripped for keyword in ["if ", "for ", "while "])
                and ")" in stripped
                and "{" not in stripped
                and not stripped.endswith(";")
            ):
                ...
                for issue in file_issues:
                    issues.append(
                        LintIssue(
                            ...
                            message=issue["text"],
                            severity=(
                                "error" if issue["code"].startswith("E") else "warning"
                            ),
                            rule=issue["code"],
                            source="flake8",
                        )
